...
export default async function BalanceSheetPage({
  ...
  // Get current and prior periods — balance sheet uses end of period
  ...
  // Fetch both current and prior period data (as of end of period)
  const [currentData, priorData] = await Promise.all([
    generateBalanceSheet(organization.id, currentPeriod.endDate),
    generateBalanceSheet(organization.id, priorPeriod.endDate),
  ]);
  ...
  const reportData = {
    ...
    prior: {
      assets: priorData.assets,
      liabilities: priorData.liabilities,
      equity: priorData.equity,
      netWorth: priorData.netWorth,
    },
    periodLabel: `As of ${currentPeriod.label} End`,
    priorPeriodLabel: `As of ${priorPeriod.label} End`,
  };
